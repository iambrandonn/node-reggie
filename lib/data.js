...
Data.prototype.loadPackage = function (pathToPackage, expectedName, expectedVersion, callback) {
  ...
  self._makeTempDir(function (err, dir) {
    if (err) {
      self._logger.error("Data.loadPackage: Failed to make directory " + dir);
      throw err;
    }
    ...
    fs.createReadStream(pathToPackage)
      ...
      .on('error', function (err) {
        self._logger.error("Data.loadPackage: Error unzipping package " + pathToPackage);
        return callback && callback.call(undefined, err);
      })
      .pipe(tar.Extract({ path: dir }))
      .on('error', function (err) {
        self._logger.error("Data.loadPackage: Error untarring package " + pathToPackage);
        return callback && callback.call(undefined, err);
      })
      .on('end', function () {
        ...
        readJson(pJsonPath, function (err, pjsonData) {
          if (err) {
            self._logger.error("Data.loadPackage: Error loading package.json " + pJsonPath + ' for ' + pathToPackage);
...
Data.prototype._registerPackageVersion = function (pjsonData, pathToPackage) {
  fs.stat(pathToPackage, function(err, stat) {
    if (err) {
      this._logger.error('Cannot stat file %s: %s', pathToPackage, err);
      return;
    }
    sha.get(pathToPackage, function(err, checksum) {
      if (err) {
        this._logger.error('Cannot compute checksum of %s: %s', pathToPackage, err);
        return;
      }
...
Data.prototype._doRegisterPackageVersion = function(pjsonData, pathToPackage, fileStat, fileChecksum) {
  ...
  this._logger.info("Registered package " + name + "@" + version);
};
